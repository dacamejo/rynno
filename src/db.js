...
async function loadFallbackStore() {
  ...
  fallbackStore.feedbackEvents = fallbackStore.feedbackEvents || [];
}
...
async function recordFeedbackEvent({
  ...
  fallbackStore.feedbackEvents.push(event);
  await persistFallbackStore();
  return event;
}

async function listFeedbackEvents({ userId = null, tripId = null, eventType = null, limit = 100 } = {}) {
  ...
  return fallbackStore.feedbackEvents
...
async function getFeedbackDashboard({ days = 30, userId = null } = {}) {
  ...
  const events = fallbackStore.feedbackEvents.filter(
